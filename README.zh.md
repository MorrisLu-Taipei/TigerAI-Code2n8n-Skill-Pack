...
> 🛠️ **責任邊界**：Hero 圖第三塊「n8n Enterprise Orchestration」裡的 SSO / IAM / HA / DR 是 **n8n self-hosted enterprise** 跟你 IT 的責任；Pack 不重做這些，但要求 Code2n8n 產出的 workflow **跑在這套基礎建設上不會破洞**（IAM-friendly、queue-safe、可 rollback）。誰做什麼、Code2n8n workflow 怎麼接，見 [`docs/enterprise-setup.md`](docs/enterprise-setup.md)。
