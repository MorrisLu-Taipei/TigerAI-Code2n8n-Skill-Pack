...
| Status (v0.34.0) | ✅ **ALSO FIXED via v3 Form native pattern** — see `workflows/einvoice-void-with-approval-v3-form-native.workflow.json`. Uses n8n Wait node `resume: form` mode (1.42+) + Email notification. Approver opens n8n-hosted HTML form via the email link, fills dropdown (approve/reject) + their email + optional note, submits → execution resumes. The form URL itself carries an n8n-generated execution token, so URL leakage gives less power than v1's bare resume URL (would-be attacker also needs to guess the active form schema). Recommended for **Taiwan & non-Slack environments** since Slack adoption in TW is low. **Optional hardening**: front the form URL with SSO / magic-link verification for sensitive approvals. |
